...
accelerate launch --config_file configs/fsdp_4gpu.yaml train_grpo.py --config configs/grpo_gsm8k.yaml

# Train DPO on GSM8K (4 GPUs) — requires data/gsm8k_preference_train.json
accelerate launch --config_file configs/fsdp_4gpu.yaml train_dpo.py --config configs/dpo_gsm8k.yaml

# Generate DPO preference pairs from GSM8K
python scripts/generate_gsm8k_pairs.py --model Qwen/Qwen2.5-7B-Instruct --output data/gsm8k_preference_train.json
...
- `data/gsm8k.py`: loads HuggingFace `openai/gsm8k`, formats into `{prompt, answer}` dicts using `PROMPT_TEMPLATE`, extracts numeric answer after `####`
- `rewards/math_reward.py`: binary reward — regex match for `#### <number>` then exact string comparison with ground truth
- `scripts/generate_gsm8k_pairs.py`: samples 2 responses per prompt, uses `math_reward` to rank them, outputs `{prompt, chosen, rejected}` JSON for DPO
